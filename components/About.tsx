...
export default function About() {
  return (
    <section id="about" style={{ backgroundColor: "#F6F3EE", padding: "96px 24px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }} className="grid-2">
          ...
          <div>
            ...
            <h2 style={{ fontSize: "clamp(26px, 3.5vw, 38px)", fontWeight: 900, color: "#1A1714", letterSpacing: "-0.02em", marginBottom: 28 }}>会社概要</h2>

            <div style={{ border: "1px solid #E4DED6", borderRadius: 3, overflow: "hidden", marginBottom: 20 }}>
              {tableRows.map((row, i) => (
                <div key={row.label} style={{ display: "grid", gridTemplateColumns: "92px 1fr", borderBottom: i < tableRows.length - 1 ? "1px solid #EDE9E2" : "none" }}>
                  <div style={{ padding: "11px 14px", backgroundColor: "#EDE9E2", fontSize: 10, color: "#A8A49E", fontWeight: 700, letterSpacing: "0.04em", display: "flex", alignItems: "flex-start", paddingTop: 13 }}>
                    {row.label}
                  </div>
                  <div style={{ padding: "11px 14px", fontSize: 12, color: "#1A1714", whiteSpace: "pre-line", lineHeight: 1.75 }}>
                    {row.value}
                  </div>
                </div>
              ))}
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {[
                ...
                <a key={sns.label} href={sns.href} target="_blank" rel="noopener noreferrer"
                  style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#636059", textDecoration: "none", border: "1px solid #E4DED6", padding: "7px 14px", borderRadius: 2, fontWeight: 600, transition: "all 0.2s", backgroundColor: "#FAFAF8" }}
                  onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "#1E3D2F"; el.style.color = "#1E3D2F"; }}
                  onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "#E4DED6"; el.style.color = "#636059"; }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={sns.icon} alt={sns.label} style={{ width: 14, height: 14, objectFit: "contain" }} />
                  {sns.label}
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <h2 style={{ fontSize: "clamp(26px, 3.5vw, 38px)", fontWeight: 900, color: "#1A1714", letterSpacing: "-0.02em", marginBottom: 28 }}>よくある質問</h2>

            <div>
              {faq.map((item, i) => (
                <div key={i} style={{ padding: "18px 0", borderBottom: i < faq.length - 1 ? "1px solid #E4DED6" : "none" }}>
                  <div style={{ display: "flex", gap: 10, marginBottom: 7 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 700, color: "#1A1714", lineHeight: 1.5 }}>{item.q}</span>
                  </div>
                  <div style={{ display: "flex", gap: 10 }}>
                    <span style={{ fontSize: 12, fontWeight: 900, color: "#1E3D2F", flexShrink: 0, lineHeight: 1.6 }}>A</span>
                    <span style={{ fontSize: 12, color: "#636059", lineHeight: 1.85 }}>{item.a}</span>
                  </div>
                </div>
              ))}
            </div>
