...
export default function Services() {
  return (
    <section id="services" style={{ backgroundColor: "#F6F3EE", padding: "96px 24px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 56, flexWrap: "wrap", gap: 20 }}>
          <div>
            ...
            <h2 style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 900, color: "#1A1714", letterSpacing: "-0.02em", lineHeight: 1.1 }}>
              サービス内容
            </h2>
          </div>
          <p style={{ fontSize: 13, color: "#636059", lineHeight: 1.9, maxWidth: 380 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }} className="grid-3">
          {services.map(({ Icon, title, desc }) => (
            <div key={title}
              style={{ backgroundColor: "#FAFAF8", border: "1px solid #E4DED6", borderRadius: 4, padding: "28px 24px", transition: "all 0.22s", cursor: "default" }}
              onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "#C4943A"; el.style.boxShadow = "0 4px 20px rgba(196,148,58,0.1)"; }}
              onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = "#E4DED6"; el.style.boxShadow = "none"; }}>
              <div style={{ color: "#1E3D2F", marginBottom: 14, opacity: 0.7 }}><Icon /></div>
              <h3 style={{ fontSize: 14, fontWeight: 800, color: "#1A1714", marginBottom: 10 }}>{title}</h3>
              <p style={{ fontSize: 12, color: "#636059", lineHeight: 1.9 }}>{desc}</p>
            </div>
          ))}
        </div>

        {/* 専門工事 */}
        <div style={{ marginTop: 12, backgroundColor: "#1E3D2F", borderRadius: 4, padding: "28px 28px", display: "flex", flexWrap: "wrap", gap: 24, alignItems: "center" }}>
          <div style={{ fontSize: 10, color: "#C4943A", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", minWidth: 80 }}>専門工事</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {["大工工事（床・天井・壁）", "家具・什器工事", "カウンター工事", "軽天・LGS工事", "水道・設備工事", "ダクト工事", "電気設備工事"].map(item => (
              <span key={item} style={{ fontSize: 11, color: "rgba(240,237,232,0.65)", border: "1px solid rgba(240,237,232,0.15)", padding: "5px 12px", borderRadius: 2 }}>{item}</span>
            ))}
          </div>
        </div>

        <div style={{ textAlign: "center", marginTop: 48 }}>
          <p style={{ fontSize: 13, color: "#A8A49E", marginBottom: 18 }}>相談・見積もりは無料です。まずはお気軽にご連絡ください。</p>
          <a href="#contact"
            style={{ display: "inline-block", backgroundColor: "#1E3D2F", color: "#F6F3EE", padding: "16px 52px", fontSize: 13, fontWeight: 800, letterSpacing: "0.08em", textDecoration: "none", borderRadius: 3, transition: "background 0.2s" }}
            onMouseEnter={e => (e.currentTarget as HTMLElement).style.backgroundColor = "#264D3C"}
            onMouseLeave={e => (e.currentTarget as HTMLElement).style.backgroundColor = "#1E3D2F"}>
            無料相談する →
          </a>
        </div>
      </div>
